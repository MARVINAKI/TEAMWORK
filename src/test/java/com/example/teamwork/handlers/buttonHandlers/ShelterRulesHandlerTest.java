...
class ShelterRulesHandlerTest {

	@InjectMocks
	private ShelterRulesHandler handler;
	@Mock
	...
	void createUpdate() {
		...
		updateCat = getUpdate(json, "/catShelterRules");
		updateDog = getUpdate(json, "/dogShelterRules");
	}
	...
	void realizationButtonCatTest() {
		...
		assertEquals(updateCat.callbackQuery().data(), "/catShelterRules");
	...
	void realizationButtonDogTest() {
		...
		assertEquals(updateDog.callbackQuery().data(), "/dogShelterRules");
		assertEquals(sendDocument.getParameters().get("chat_id"), BTN_FROM_ID);
		assertNull(sendDocument.getParameters().get("text"));
	}
}