package com.example.teamwork.handlers.messageHandlers.dog;
...
@Component
public class DogReportHandler implements MessageWithStatusHandler {
	...
	public DogReportHandler(TelegramBot telegramBot, ReportService reportService) {
	...
	public boolean checkMessage(Update update, Status status) {
		return update.message() != null && status.getDescription().equals("/dogShelterGetReport");
	}
	...
	public void realizationMessage(Update update) {
		...
		keyboardMarkup.addRow(new InlineKeyboardButton("Попробовать снова").callbackData("/dogShelterGetReport"));
